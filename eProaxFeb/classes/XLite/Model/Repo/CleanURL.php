...
class CleanURL extends \XLite\Model\Repo\ARepo
{
    ...
    public function parseURL($url, $last = '', $rest = '', $ext = '')
    {
        ...
        foreach ($this->getEntityTypes() as $model => $entityType) {
            $method = $entityType
                ? __FUNCTION__ . \XLite\Core\Converter::convertToCamelCase($entityType)
                : null;
    ...
    protected function prepareBuildURL($target, $params, $urlParts)
    {
        if ('product' == $target) {
            ...
            if (!empty($params['category_id']) && $repo->hasProduct($params['category_id'], $params['product_id'])) {
     ...
     * @return string
     */
    public function buildFakeURL($entity, $params = array())
    {
        $result = '';

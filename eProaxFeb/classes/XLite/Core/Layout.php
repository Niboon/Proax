...
class Layout extends \XLite\Base\Singleton
{
    ...
    public function getResourceFullPath($shortPath, $interface = null)
    {
        ...
        if (!isset($this->resourcesCache[$key])) {
            foreach ($this->getSkinPaths($interface, $this->isModuleResource($shortPath)) as $path) {
    ...
    {
        $found = false;
        $paths = $this->getSkinPaths($this->currentInterface, $this->isModuleResource($shortPath));
        reset($paths);
    ...
    public function getResourceSkinFullPath($shortPath, $skin, $interface = null)
    {
        ...
        foreach ($this->getSkinPaths($interface ?: $this->currentInterface, $this->isModuleResource($shortPath)) as $path) {
            if ($path['name'] == $skin) {
                $fullPath = $path['fs'] . LC_DS . $shortPath;
                if (file_exists($fullPath)) {
                    $result = $fullPath;
    ...
    {
        $skins = $this->getSkinPaths($this->currentInterface, $this->isModuleResource($shortPath));
    ...
    public function getSkinPaths($interface = null, $isModuleResource = false, $reset = false)
    {
        ...
        $key .= (string) $isModuleResource;

        if (!isset($this->skinPaths[$key]) || $reset) {
            ...
            foreach ($this->getSkins($interface) as $skin) {
                foreach ($locales as $locale) {
                    ...
                    if ($commonInterface) {
                        $mailCommonFsPath = !$isModuleResource ? LC_DS . 'mail' : '';
                        $mailCommonWebPath = !$isModuleResource ? '/' . 'mail' : '';
    ...
    /**
     * Is resource belongs to module
     *
     * @param string $shortPath Short path to resource
     *
     * @return boolean
     */
    protected function isModuleResource($shortPath)
    {
        return 0 === strpos($shortPath, 'modules');
    }
    ...
    {
        return !empty($data['file']) && preg_match('/\.less$/Ss', $data['file']);
    }
